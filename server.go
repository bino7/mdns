...
func NewServer(config *Config) (*Server, error) {
	...
	l4, _ := net.ListenUDP("udp4", ipv4Addr)
	l6, _ := net.ListenUDP("udp6", ipv6Addr)

	// Check if we have any listener
	if l4 == nil && l6 == nil {
		return nil, fmt.Errorf("no listeners could be started")
	}

	s := &Server{
		config:     config,
		ipv4List:   newPacketConn(l4),
		ipv6List:   newIPV6PacketConn(l6),
	...
	if s.ipv6List != nil {
		go s.recvIPV6(s.ipv6List)
	}

	return s, nil
}

func newPacketConn(l *net.UDPConn) *ipv4.PacketConn {
	...
	for i := 0; i < len(ifaces); i++ {
		if err := pc.JoinGroup(&ifaces[i], ipv4Addr); err != nil {
			continue
		}
	}
	return pc
}
func newIPV6PacketConn(l *net.UDPConn) *ipv6.PacketConn {
...
func (s *Server) recv(c *ipv4.PacketConn) {
	if c == nil {
		return
	}
	buf := make([]byte, 65536)
	for !s.getIsShutdown() {
		...
		if err != nil {
			log.Errorf("[ERR] mdns: Failed to read: %v", err)
			continue
		}
...
func (s *Server) recvIPV6(c *ipv6.PacketConn) {
	if c == nil {
		return
	}
	buf := make([]byte, 65536)
...
func (s *Server) handleQuery(query *dns.Msg, from net.Addr) error {
	...
	if len(multicastAnswer) == 0 && len(unicastAnswer) == 0 {
		...
		log.Errorf("no responses for query with questions: %s", strings.Join(questions, ", "))
	}
